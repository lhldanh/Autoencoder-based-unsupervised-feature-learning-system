// =============================================================================
// ASYNC MEMORY TRANSFER OPTIMIZATION
// =============================================================================
// Kỹ thuật: Sử dụng cudaMemcpyAsync + CUDA Streams + Pinned Memory + Double Buffering
// để overlap data transfer với computation.
// 
// Luồng hoạt động:
// - Batch N đang được GPU xử lý (compute stream)
// - Đồng thời Batch N+1 đang được transfer từ CPU → GPU (transfer stream)
// - Khi batch N xong, batch N+1 đã sẵn sàng → không có idle time
//
// So sánh:
// - Synchronous: [Transfer][Compute][Transfer][Compute]...
...
int main() {
    ...
    // ============== DOUBLE BUFFERING FOR ASYNC TRANSFER ==============
    // Sử dụng 2 buffer để overlap transfer và compute
    float *d_input[2];
    ...
    // ============== PINNED MEMORY FOR FASTER ASYNC TRANSFER ==============
    // Pinned memory cho phép DMA transfer không cần CPU intervention
    // Điều kiện bắt buộc để cudaMemcpyAsync hoạt động đúng
    float* h_pinned_input[2];
    cudaMallocHost(&h_pinned_input[0], s_in * 4);  // Pinned buffer 0
    cudaMallocHost(&h_pinned_input[1], s_in * 4);  // Pinned buffer 1
    ...
    // Stream compute: Chạy các kernel tính toán
    // Stream transfer: Chạy các async memory copy
    cudaStream_t stream_compute, stream_transfer;
    cudaStreamCreate(&stream_compute);
    cudaStreamCreate(&stream_transfer);
    
    // CUDA Event để đồng bộ giữa streams
    cudaEvent_t transfer_done[2];
    cudaEventCreate(&transfer_done[0]);
    cudaEventCreate(&transfer_done[1]);
    ...
    for (int epoch = 0; epoch < EPOCHS; ++epoch) {
        ...
        // Copy batch đầu tiên vào pinned memory rồi async transfer
        memcpy(h_pinned_input[0], dataset.get_train_images_ptr(), s_in * 4);
        cudaMemcpyAsync(d_input[0], h_pinned_input[0], s_in * 4, 
                       cudaMemcpyHostToDevice, stream_transfer);
        cudaEventRecord(transfer_done[0], stream_transfer);
        ...
        for (int b = 0; b < num_batches; ++b) {
            ...
            // Trong khi GPU đang tính batch hiện tại, ta async transfer batch tiếp theo
            if (b + 1 < num_batches) {
                size_t next_offset = (size_t)(b + 1) * s_in;
                // Copy vào pinned memory (CPU operation)
                memcpy(h_pinned_input[next_buf], dataset.get_train_images_ptr() + next_offset, s_in * 4);
                // Async transfer từ pinned memory → GPU
                cudaMemcpyAsync(d_input[next_buf], h_pinned_input[next_buf], s_in * 4, 
                               cudaMemcpyHostToDevice, stream_transfer);
                cudaEventRecord(transfer_done[next_buf], stream_transfer);
            }
            
            // ============== WAIT FOR CURRENT BATCH TRANSFER ==============
            // Đảm bảo batch hiện tại đã được transfer xong trước khi compute
            cudaStreamWaitEvent(stream_compute, transfer_done[curr_buf], 0);

            // FORWARD on compute stream
            conv2d_kernel<<<get_1d_dims(s_l1), 256, 0, stream_compute>>>(d_input[curr_buf], d_w1, d_b1, d_l1, p1);
            ...
            // BACKWARD
            mse_backward_kernel<<<get_1d_dims(s_in), 256, 0, stream_compute>>>(d_out, d_input[curr_buf], d_dout, s_in);
            ...
            conv2d_backward_weight_kernel<<<get_1d_dims(h_w1.size()), 256, 0, stream_compute>>>(d_dl1, d_input[curr_buf], d_dw1, p1);
    ...
    std::cout << "⏱️  train_gpu_optimize_async_transfer: " << std::chrono::duration<double>(t_end - t_start).count() << "s\n";
    
    // Cleanup
    cudaEventDestroy(transfer_done[0]);
    cudaEventDestroy(transfer_done[1]);
    cudaFreeHost(h_pinned_input[0]);
    cudaFreeHost(h_pinned_input[1]);
