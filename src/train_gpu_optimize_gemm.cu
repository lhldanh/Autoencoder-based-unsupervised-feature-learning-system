...
int main() {
    ...
    float *d_input, *d_l1, *d_p1, *d_l2, *d_p2, *d_l3, *d_u3, *d_l4, *d_u4, *d_out;
    ...
    // Col buffers for GEMM optimization
    ...
    cudaMalloc(&d_col1, col1_size * 4);
    cudaMalloc(&d_col2, col2_size * 4);
    cudaMalloc(&d_col3, col3_size * 4);
    cudaMalloc(&d_col4, col4_size * 4);
    cudaMalloc(&d_col5, col5_size * 4);
    ...
    for (int epoch = 0; epoch < EPOCHS; ++epoch) {
        for (int b = 0; b < num_batches; ++b) {
            ...
            // FORWARD using im2col + GEMM optimization
            // Layer 1: input -> l1 (Conv + ReLU via GEMM)
            ...
            // Layer 2: p1 -> l2 (Conv + ReLU via GEMM)
            ...
            // Layer 3: p2 -> l3 (Conv + ReLU via GEMM)
            ...
            // Layer 4: u3 -> l4 (Conv + ReLU via GEMM)
            ...
            // Layer 5: u4 -> out (Conv without ReLU)
    ...
    std::cout << "⏱️  train_gpu_optimize_gemm: " << std::chrono::duration<double>(t_end - t_start).count() << "s\n";
