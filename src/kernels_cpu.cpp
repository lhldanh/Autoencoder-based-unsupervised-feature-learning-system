...
void upsample_cpu(float* input, float* output, int batch, int in_h, int in_w, int in_c) {
    int out_h = in_h * 2;
    int out_w = in_w * 2;
    // Map output pixel (oh, ow) -> input pixel (oh/2, ow/2)
    for (int b = 0; b < batch; ++b) {
        for (int c = 0; c < in_c; ++c) {
            for (int oh = 0; oh < out_h; ++oh) {
                for (int ow = 0; ow < out_w; ++ow) {
...
// 1. Convolution Backward [cite: 258, 318]
// d_output: Gradient từ lớp sau truyền về
// input: Input của lớp hiện tại (đã lưu từ forward)
// d_input: Gradient cần tính để truyền về lớp trước
// d_weight, d_bias: Gradient cần tính để update trọng số
void conv2d_backward_cpu(float* d_output, float* input, float* weight, 
                         float* d_input, float* d_weight, float* d_bias, ConvParam p) {
    
    // Reset gradients về 0
        ...
        for (int oh = 0; oh < p.out_h; ++oh) {
            for (int ow = 0; ow < p.out_w; ++ow) {
                for (int oc = 0; oc < p.out_c; ++oc) {
                    ...
                    // 1. Gradient cho Bias: Cộng dồn gradient output
                    d_bias[oc] += d_val;
                    ...
                    for (int ic = 0; ic < p.in_c; ++ic) {
                        for (int kh = 0; kh < p.k_size; ++kh) {
                            for (int kw = 0; kw < p.k_size; ++kw) {
                                ...
                                if (ih >= 0 && ih < p.in_h && iw >= 0 && iw < p.in_w) {
                                    // 2. Gradient cho Weights: Input * d_output
                                    int in_idx = get_idx(b, ih, iw, ic, p.in_h, p.in_w, p.in_c);
                                    int w_idx = oc * (p.in_c * p.k_size * p.k_size) 
                                              + ic * (p.k_size * p.k_size) 
                                              + kh * p.k_size + kw;
                                    ...
                                    // 3. Gradient cho Input: d_output * Weight
                                    // (Tích luỹ gradient từ output truyền về input)
...
// 2. ReLU Backward 
// Gradient chỉ truyền qua nếu input > 0
...
// 3. MaxPool Backward 
// Gradient chỉ truyền về đúng vị trí Max, các vị trí khác bằng 0
        ...
        for (int c = 0; c < in_c; ++c) {
            for (int oh = 0; oh < out_h; ++oh) {
                for (int ow = 0; ow < out_w; ++ow) {
                    ...
                    // Tìm lại vị trí Max trong cửa sổ forward
                    int max_idx = -1;
                    float max_val = -1e9;
                    ...
                    // Truyền gradient về đúng vị trí đó
                    int out_idx = get_idx(b, oh, ow, c, out_h, out_w, in_c);
                    if (max_idx != -1) {
                        d_input[max_idx] += d_output[out_idx];
                    }
                }
            }
        }
